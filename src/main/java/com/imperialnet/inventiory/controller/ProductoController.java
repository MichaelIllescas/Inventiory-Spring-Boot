...
public class ProductoController {
                                          ...
                                          Model model)
    {
        ...
        for (Producto prod : productosAumento) {
            prod.setPrecio(prod.getPrecio() + (prod.getPrecio() * porcentaje) / 100);
            prodServ.crearProducto(prod, sesion);
        }

        listado.vaciar();
        sesion.removeAttribute("productosSeleccionados");
        listado.vaciar();
        productosAumento.clear();
        sesion.setAttribute("confirmacion", "Se aplicó el aumento correctamente");
        return "redirect:/aplicarAumentos";
                                    ...
                                    Model model)
    {
        ...
        if (idUsuario == null) {
            // Manejar el caso donde idUsuario es null, por ejemplo redirigir a la página de inicio de sesión
            return "redirect:/login";
        }
        List<Producto> productos = prodServ.obtenerProductosPorUsuario(idUsuario);
        model.addAttribute("productos", productos);

        listado.vaciar();
        return "aplicarDescuentos";  // Esto debe coincidir con el nombre del archivo HTML en el directorio de plantillas
    }
            ...
            HttpSession sesion) 
    {
        ...
        // Actualizar la lista de productos seleccionados y el total de la venta en la sesión
        sesion.setAttribute("productosSeleccionados", listado.getListadoAumento());
        return "redirect:/aplicarDescuento"; // Redirige al formulario de registro de venta
    }
                                      ...
                                      HttpSession sesion)
    {
        ...
        if (productoSeleccionado == null) {
            sesion.setAttribute("errorProd", "El producto seleccionado no existe.");
            return "aplicarDescuentos"; // Redirige al formulario de registro de venta
        }
    ...
    @PostMapping("/confirmarDescuento")
    @Transactional
            ...
            Model model) 
    {
        ...
        // Validar si se seleccionaron productos para la venta
        if (productosAumento == null || productosAumento.isEmpty()) {
            sesion.setAttribute("errorProd", "No se han seleccionado productos para aplicar el descuento.");
            sesion.removeAttribute("confirmacion");
        ...
        listado.vaciar();
        productosAumento.clear();
        sesion.setAttribute("confirmacion", "Se aplicó el descuento correctamente");
        return "redirect:/aplicarDescuento";
    }
}
